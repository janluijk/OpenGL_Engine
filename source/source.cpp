...
int main() {
  ...
  unsigned int cube_indices[] = {0,  1,  2,  0,  2,  3,
  ...
  float plane_vertices[] = {-1.0f, 0.0f, -1.0f, 0.0f, 1.0f, 0.0f, -1.0f, 0.0f,
                            1.0f,  0.0f, 1.0f,  0.0f, 1.0f, 0.0f, -1.0f, 0.0f,
                            1.0f,  0.0f, 1.0f,  0.0f, 1.0f, 0.0f, 1.0f,  0.0f};

  unsigned int plane_indices[] = {0, 1, 2, 1, 2, 3};
  ...
  glm::vec3 lightPositions[] = {glm::vec3(0.0), glm::vec3(0.0), glm::vec3(0.0)};
  ...
  while (!glfwWindowShouldClose(window)) {
    ...
    glBindVertexArray(cubeVAO); // seeing as we only have a single VAO there's
    ...
    for (int x = -5; x < 5; x++) {
      for (int y = -5; y < 5; y++) {
        for (int z = -5; z < 5; z++) {
          ...
          if ((x + y + z) % 15 == 0) {
            model = glm::rotate(model, 0.05f * gl_time,
    ...
    // This function checks for events and activates the callback functions on
    // events
    glfwPollEvents();
  }
